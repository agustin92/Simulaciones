...
def open_data_RUNS(folder):
	...
	for i in range(L):
		...
		presion_gas_ideal = temperature[i]*densidad[i]
		N = 200
		factor = (N**2 - N)/2

		#presion[i] = (data[1,2] - presion_gas_ideal)*factor + presion_gas_ideal
		presion2[i] = data[1,3] #ignorar, porque esta mal
		presion_var[i] = data[1,4] #ignorar, porque esta mal
		...
		temperature[i] = np.mean(data_energy[1:,5]) #tomo de la instantaea

		presion[i] = np.mean( (data_energy[1:,4] - data_energy[1:,5]*densidad[i])*factor + data_energy[1:,5]*densidad[i] ) #tomo de la instantaea
...
def plot_data_densidad(save_folder):
	...
	plt.plot(densidad, presion, marker = 'o', linestyle = '--')
	plt.xlabel('Densidad')
...
if __name__ == '__main__':
    ...
    parent_folder = 'D:/Mis Cosas/Mis documentos/compu/Facultad/Materias posgrado/data_SimulacionesMD/Temperatura'
