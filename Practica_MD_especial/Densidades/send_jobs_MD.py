...
for renglon in f:
    ...
    for i in range(11):
        if i == 0 :
            ...
            process5 = subprocess.run('mv variables_fisicas.dat mean_measurement.dat '+path_directory+'/{}/RUN_{}'.format(densidad,i+1),shell=True)
            process6 = subprocess.run('cp configuracion_ini.dat '+path_directory+'/{}/RUN_{}'.format(densidad,i+1),shell=True)
            process7 = subprocess.run('cp input_pos.dat input_vel.dat '+path_directory+'/{}/RUN_{}'.format(densidad,i+1),shell=True)
            ...
            if i == 10:
                process3 = subprocess.run('mv variables_fisicas.dat mean_measurement.dat movie.vtf '+path_directory+'/{}/RUN_{}'.format(densidad,i+1),shell=True)
            else:
                process3a = subprocess.run('mv variables_fisicas.dat mean_measurement.dat '+path_directory+'/{}/RUN_{}'.format(densidad,i+1),shell=True)
                process3b = subprocess.run('rm movie.vtf',shell=True)
            process4 = subprocess.run('cp configuracion_ini.dat '+path_directory+'/{}/RUN_{}'.format(densidad,i+1),shell=True)
            process5 = subprocess.run('cp input_pos.dat input_vel.dat '+path_directory+'/{}/RUN_{}'.format(densidad,i+1),shell=True)
            print('Finalizacion de simulacion {} de MD para densidad {}'.format(i+1,densidad))                
f.close()        